#!/usr/bin/env python
"""Frozen-model passes for a Gate-8 training source: LingBot native streaming, the
calibrated-FOV MoGe-B depth per frame, and the per-frame metric-gauge candidate.

Identical settings to Gate 7B (same replay code, same MoGe wrapper, same estimator); the
only new thing is the source. Nothing opens a target.

    python tools/gate8/stream_sources.py --source sk_train --device cuda:1
"""
...
def main() -> int:
    ...
    for sub in ("stream", "moge_b", "scale"):
        os.makedirs(f"{S.G8_ROOT}/{sub}/{a.source}", exist_ok=True)
    ...
    for seg in segs:
        ...
        # MoGe-B per frame + scale candidate
