#!/usr/bin/env python
"""Gate 8D: cache (causal input, dense target) training pairs for one KITTI-360 drive.

Identical to the Gate 8C-1 builder in every causal respect -- same frozen 32-channel export,
same mapper, same scale fixed from the first five frames, every frame integrated exactly
once -- and different in exactly two places:

* the geometry target is ``gate8d/densetarget.py``'s fused LiDAR + MoGe + sky volume;
* each sample additionally carries the truncated unsigned surface distance in **metres**
  and its validity mask, for the Phase 4 auxiliary head.

The semantic target is unchanged: frozen Trident-H fused over the future window through the
same frozen mapper, kept only where geometrically supported and where the two halves of the
window agree on the top-1 class. No human label anywhere.
...
from gates.gate8d import densetarget as DT                           # noqa: E402
...
def main() -> int:
    ...
    for i in range(len(seg)):
        ...
        tgt = DT.load(SRC.target_path(a.drive, i))
        ...
        smp = {
            ...
            "n_obs_target": tgt["n_obs"], "n_frames_occ_target": tgt["n_frames_occ"],
                "tudf_m": tgt["tudf_m"].astype(np.float16),
                "tudf_valid": np.packbits(tgt["tudf_valid"]),
