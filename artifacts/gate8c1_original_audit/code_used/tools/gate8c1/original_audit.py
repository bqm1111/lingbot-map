...
def prepare():
    ...
    from gate6.grids import PREDICTION_GRID, EVAL_GRID
    import subprocess
    ...
    files = list((ROOT / "gate8").glob("*.py")) + [ROOT / p for p in (
        "tools/gate8c1/eval_target.py", "gate6/targets.py", "gate6/grids.py",
        "gate8a/regions.py", "gate8a/scores.py", "gate7b/replay.py",
        "gate7b/streams.py", "lingbot_map/models/gct_stream.py")]
    for p in files:
        record(p)
...
def reduce_binary(p, ds):
    from gate8a.regions import occ_to_eval_grid
    from gate6.grids import EVAL_GRID
...
def infer(ds, mode, device, verify_only=False):
    ...
    from gate8.net import load_checkpoint
    from gate6.grids import PREDICTION_GRID
    from tools.gate8c1.eval_target import window_map
    from voxel_gate.voxels import dilate
...
def analyze(ds, mode, verify_only=False):
    ...
    from gate6 import targets as old_targets
    from gate6.grids import EVAL_GRID
    from gate8c1.occany_eval import official_sc_counts
    doc = json.loads((OUT / "samples.json").read_text())[ds][mode]
    rows = doc["rows"]
    if verify_only: rows = [r for r in rows if r["clip_id"] in doc["verification_ids"]]
...
def provenance():
    ...
    from gate6 import frames as F
    from gate8 import sources as S
    from gate7b.scale import frame_candidate
    from gate7b.depth import unpack_mask
...
def finish():
    ...
    from PIL import Image
    from gate6.grids import EVAL_GRID
    from gate6 import metrics as G6M, vocab as G6V
    from gate8c1.occany_eval import official_sc_counts
    allresults={}
...
def mechanism(device):
    ...
    import torch
    from gate8.net import load_checkpoint
    from gate6.grids import PREDICTION_GRID, EVAL_GRID, voxelize
